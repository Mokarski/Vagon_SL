...
/* Function to convert 4bit number to HEX char */
...
uint16_t gcrc16(uint8_t *buffer, uint16_t buffer_length)
{
	return crc16(buffer, buffer_length);
        ...
        /// </summary>
static uint8_t crcascii(const uint8_t *buffer, uint16_t buffer_length)
{
            ...
            lrc = (uint8_t) ((lrc ^ 0xFF) + 1);

            return lrc;
}

uint8_t gcrcascii(uint8_t *buffer, uint16_t buffer_length)
{
	const uint8_t *bf = buffer;
	return crcascii(bf, buffer_length);
}		
...
{
if (ctx->rtumode == 1) { // RTU mode send
...
// ASCII mode send
   uint16_t asciilen = 1 + 2 *(req_length - 2) + 2 + 2;
   uint8_t *asciibuff = ctx->asciibuffwrite;
   uint8_t lrc = crcascii(req, req_length - 2);
   asciibuff[0] = ':';
   asciibuff[asciilen - 4] = numtohex((lrc >> 4) & 0x0F);
   asciibuff[asciilen - 3] = numtohex(lrc & 0x0F);
   asciibuff[asciilen - 2] = '\r';
   asciibuff[asciilen - 1] = '\n';
   uint16_t i = 1;
   uint16_t j = 0;
   while(j < (req_length - 2)) {
	   asciibuff[i] = numtohex((req[j] >> 4) & 0x0F);
	   ++i;
	   asciibuff[i] = numtohex(req[j] & 0x0F);
	   ++i;
	   ++j;
   }
   ...
   {
	   printf("Write - ");
	   for(j = 0; j < asciilen -2; ++j)
		   printf("%c",asciibuff[j]);
		   printf("\n");
   }
   ssize_t wrc = 0;
#if defined(_WIN32)
    modbus_rtu_t *ctx_rtu = ctx->backend_data;
    DWORD n_bytes = 0;
    wrc = (WriteFile(ctx_rtu->w_ser.fd, asciibuff, asciilen, &n_bytes, NULL)) ? n_bytes : -1;
#else
    wrc =  write(ctx->s, asciibuff, asciilen);
#endif
	if (wrc == asciilen) return req_length;
	if (wrc == -1) return -1;
   return wrc >> 1;
...
// Function to set RTU mode
void modbus_rtu_set_rtu_mode(modbus_t *ctx){
	
ctx->rtumode = 1;
modbus_rtu_t *ctx_rtu = (modbus_rtu_t *)ctx->backend_data;
ctx_rtu->rtumode = ctx->rtumode;
free(ctx->asciibuffread);
free(ctx->asciibuffwrite);
ctx->asciibuffread = ctx->asciibuffwrite = NULL;
                         ...
                         int stop_bit)
{
	...
	// Setting ASCII mode to default
	ctx->rtumode = 0;
    ctx_rtu->rtumode = ctx->rtumode;
	// Locate ASCII buffer of max size of packet
	ctx->asciibuffread = malloc(MAX_ASCII_LENGTH_PACKET);
	ctx->asciibuffwrite = malloc(MAX_ASCII_LENGTH_PACKET);
