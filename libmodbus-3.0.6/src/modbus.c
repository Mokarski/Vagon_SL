...
static int receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
{
	...
	uint8_t *asciibuff = ctx->asciibuffread;
	int timek = 1;
	
	
	if(ctx->rtumode == 0) { // ASCII mode is ON
		length_to_read = MAX_ASCII_LENGTH_PACKET;
		timek = 2;
		memset(asciibuff, 0, length_to_read);
	}
    ...
    if (msg_type == MSG_INDICATION) {
        ...
        tv.tv_sec = ctx->response_timeout.tv_sec * timek;
        tv.tv_usec = ctx->response_timeout.tv_usec * timek;
        p_tv = &tv;
    }
    ...
    while (length_to_read != 0) {
        ...
        if (ctx->rtumode == 1)
			...
			rc = ctx->backend->recv(ctx, asciibuff + msg_length, length_to_read);
        ...
        if (ctx->debug)  {
            ...
            for (i=0; i < rc; i++)
				...
				/*
				for (i=0; i < rc; i++) {
					switch(asciibuff[msg_length + i])
					{
						default: printf("[%c]", asciibuff[msg_length + i]);
						break;
						case '\n': case '\r': break;
					}
        ...
        if (ctx->rtumode == 1) { // RTU mode code
			if (length_to_read == 0) {
				switch (step) {
				case _STEP_FUNCTION:
		...
		} else // ASCII mode code
		{
			...
			if (sem > 0) { // Left shift data in buffer if some junk present before packet
				j = 0;
				for(i = sem; i < msg_length; ++i)
				{
					asciibuff[j] = asciibuff[i];
					++j;
				}
				msg_length -= sem;
				length_to_read += sem;
				if (length_to_read > MAX_ASCII_LENGTH_PACKET) length_to_read = MAX_ASCII_LENGTH_PACKET;
				sem = 0;
			}
			if (sem == 0) // found start of packet
			{
				int found = 0;
				for(i = 1; i < msg_length - 1; ++i)
				{
				  if ((asciibuff[i] == '\r') && (asciibuff[i+1] == '\n'))
				  {
				     found = i;
					 break;
				  }
				}
				if (found > 0) // Found valid ASCII packet
				{
					if (ctx->debug)
					{
			            printf("Read - ");
						for (i=0; i < msg_length -2; i++)
                           printf("%c", asciibuff[i]);
						   printf(" --- ");
					}
					uint8_t *rawmsg = malloc(msg_length + 2);
					uint16_t rawmsglen = 0;
					j = 0; i = 1;
					while(i < found)
					{
						rawmsg[j] = hextonum(asciibuff[i]) << 4;
						++i;
						rawmsg[j] |= hextonum(asciibuff[i]);
						++i;
						++rawmsglen;
						++j;
					}
					if (rawmsg[rawmsglen - 1] == gcrcascii(rawmsg, rawmsglen - 1)) // LRC ASCII is correct
					{
					    --rawmsglen;
						uint16_t crc = gcrc16(rawmsg, rawmsglen);	
						rawmsg[rawmsglen++] = crc >> 8;
						rawmsg[rawmsglen++] = crc & 0x00FF;
						memcpy(msg, rawmsg, rawmsglen);
						msg_length = rawmsglen;
						length_to_read = 0;	
					    if (ctx->debug)
				        	{
						      printf("LRC valid\n");
						...
						if (ctx->debug) {
							fprintf(stderr, "ERROR CRC received %0X != LRC calculated %0X\n",
									rawmsg[rawmsglen - 1], gcrcascii(rawmsg, rawmsglen - 1));
						}
        ...
        if (length_to_read > 0 && ctx->byte_timeout.tv_sec != -1) {
            /* If there is no character in the buffer, the allowed timeout
               ...
               byte_timeout */
            tv.tv_sec = ctx->byte_timeout.tv_sec * timek;
            tv.tv_usec = ctx->byte_timeout.tv_usec * timek;
            p_tv = &tv;
        }
    }
...
void modbus_free(modbus_t *ctx)
{
    ...
    if (ctx->asciibuffread) free(ctx->asciibuffread);
	if (ctx->asciibuffwrite) free(ctx->asciibuffwrite);
